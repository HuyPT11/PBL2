...
Staff::Staff()
{
    this->ID = this->Name = this->Sex = this->Phonenumber = this->Address = "";
    this->Age = 0;
    this->Salary = 0;
...
Staff::Staff(const Staff &)
      :ID(ID),Name(Name),Sex(Sex),Age(Age),Phonenumber(Phonenumber),Address(Address),Salary(Salary)
{
...
ostream &operator<<(ostream &o, Staff b)
{
    o << 
    return o;
}
istream &operator>>(istream &i, Staff &b)
{
    i >> 
    return i;
}
